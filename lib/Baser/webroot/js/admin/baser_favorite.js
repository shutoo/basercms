$(function(){
	$("body").append($("#BcFavoritesMenu"));
	$("#BtnFavoriteAdd").click(function(){
		$('.bc-favorite-dialog').dialog('open');
		return false;
	});
/**
 * お気に入り初期化
					...
					$("#FavoriteAjaxForm").ajaxSubmit({
						...
						success: function(response, status) {
							if(response) {
								...
								initFavoriteList();
								$('.bc-favorite-dialog').dialog('close');
							} else {
								alert('保存に失敗しました。');
							}
						},
 ...
 */
	function favoriteSortUpdateHandler(event, ui){
		...
		$.ajax({
			url: $(".bc-favorite-ajax-sorttable-url").html(),
			type: 'POST',
			data: data,
			dataType: 'text',
			beforeSend: function() {
 ...
 */
	function initFavoriteList() {
		...
		$(".favorite-menu-list li").contextMenu({menu: 'BcFavoritesMenu'}, contextMenuClickHandler);
		...
		switch (action){
			...
			case 'FavoriteDelete':
				if(confirm('本当に削除してもよろしいですか？')){
					$("#Waiting").show();
					$.post($(".bc-favorite-delete-url").html(), {"data[Favorite][id]": $(".favorite-menu-list .selected .favorite-id").val()}, function(result){
						if(result){
